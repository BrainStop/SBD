...
public class Manipula {
	...
	/**
	 * Construtor para estabelecer a configuração de acesso á base de dados a
	 * partir de um objecto da classe Configura
	 * 
	 * @param cfg
	 *            Objecto com a configuração
	 */
	 ...
	 *            Driver JDC
	 * @param url
	 *            URL para acesso á base de dados
	 * @param usr
	...
	/**
	 * Fecha a conexão e a directiva associada á classe. Deve fazer-se sempre
	 * que termine o acesso aos dados.
	 * 
	 ...
	 * 
	 * @param directivas
	 *            Conjunto de directivas SQL (não pode ter SELECT)
	...
	public boolean executaBatch(String directivas[]) {
		...
		try {
			...
			System.err
			.println("Falhou a consulta da metada e/ou não existe suporte para a execução de 'batch'.");
		}
		if (ok) {
			...
			System.err
			.print("O Driver não suporta a execução de directivas em 'batch'");
			System.err
			.println("O processamento vai ser feito executando directivas separadas");
			for (int i = 0; i < directivas.length; i++) {
				try {
	...
	public boolean exportar(String tabela) {
		try {
			...
			while (rs.next()) {
				String Insert = "INSERT INTO " + tabela + " (" + colNames
				+ ") VALUES (";
				for (int i = 1; i <= cols; i++) {
					Insert = Insert
					+ cfg.fmTipo(rs.getObject(i), typecols[i - 1],
							typenames[i - 1]) + ", ";
				}
	...
	/**
	 * Retorna uma mensagem relativa ao número de linhas afectadas na execução
	 ...
	 * afectada. No caso da execução de isntruções SQL DDL (CREATE, ALTER, DROP)
	 * é sempre devolvido false
	 * 
	...
	/**
	 * Regista uma única vez o driver JDBC para efectuar o acesso á base de
	 * dados
	 */
	public void Register() {
		if (!done)
			try {
				...
				System.err.println(e.toString());
				System.err
				.println("Não é possivel estabelecer a ligação com a base de dados.");
				System.err.println("Verifique o Host/ IP e a autenticação\r\n");
				System.out
				.println("Veja as mensagens seguintes com uma descrição completa do erro.");
				e.printStackTrace();
				desligar();
			}
	}
